...
export const FeedbackModal: React.FC<FeedbackModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
      <div className="relative w-full max-w-md bg-slate-900 rounded-3xl border border-slate-800 shadow-2xl p-6 sm:p-7 overflow-hidden animate-in fade-in zoom-in-95 duration-200 text-white">
        {/* Botón cerrar */}
        <button
          onClick={onClose}
        ...
        {isSuccess ? (
          <div className="text-center py-8">
            <div className="w-14 h-14 bg-emerald-950 text-emerald-400 border border-emerald-800/80 rounded-full flex items-center justify-center mx-auto mb-3 shadow-inner">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="text-center">
              <span className="inline-flex items-center gap-1 text-[11px] font-bold text-blue-400 bg-blue-950/60 px-2.5 py-0.5 rounded-full border border-blue-800/80 mb-1.5">
                ...
                return (
                  <button
                    ...
                    className={`text-[11px] font-medium px-2.5 py-1 rounded-xl border transition-all ${
                      isSelected
                        ? 'border-blue-500 bg-blue-950/80 text-blue-200 font-bold'
            ...
            <div>
              <textarea
                ...
                rows={3}
                maxLength={300}
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-700 bg-slate-800/80 text-white text-xs placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-600 transition-all resize-none"
              />
            ...
            <button
              ...
              className="w-full py-3.5 px-4 rounded-xl bg-gradient-to-r from-red-600 to-blue-700 text-white font-bold text-xs tracking-wide shadow-md hover:shadow-lg disabled:opacity-50 transition-all flex items-center justify-center gap-2 cursor-pointer"
            >
